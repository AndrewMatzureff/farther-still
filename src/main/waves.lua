...
waves = {
    util = {fourier = {}},
    data = {fourier = {}},
    prefabs = {fourier = {}, functions = {}}
}

-- util

function waves.util.fourier.coefficients(count, seed, fun)
	local coefficients = {}
	local e = seed
	for i = 1, count, 1 do
		e = fun(e)
...
function waves.util.fourier.get(x, phase, offset, prefab, precision)
	local y = 0.0
...
waves.util.fourier.sample = sin--function (angle) return sin(angle / TWO_PI) end
...
function waves.prefabs.functions.funnel(x) return min(2^x, 2^-x) end
